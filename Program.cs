...
class MainForm : Form
{
    ...
    void RenderQuiz()
    {
        ...
        var translation = target.exampleZh; var shuffled = options.OrderBy(_ => random.Next()).ToList(); for (var optionNumber = 0; optionNumber < shuffled.Count; optionNumber++) { var option = shuffled[optionNumber]; var button = new Button { Text = $"{(char)('A' + optionNumber)}. {words[option].en}", AutoSize = true, Tag = option, MinimumSize = new Size(360, 42), TextAlign = ContentAlignment.MiddleLeft }; StyleButton(button); button.Click += (_, _) => { var correct = (int)button.Tag! == targetIndex; button.BackColor = correct ? Color.FromArgb(220, 252, 231) : Color.FromArgb(254, 226, 226); button.ForeColor = correct ? Color.FromArgb(22, 101, 52) : Color.FromArgb(153, 27, 27); quizTranslation.Text = $"題目句子翻譯：{translation}"; quizTranslation.Visible = true; feedback.Text = correct ? $"答對了！「{target.en}」：{target.zh}" : $"答案：{target.en}　{target.zh}"; choices.PerformLayout(); LayoutQuiz(); }; choices.Controls.Add(button); } LayoutQuiz();
    }
